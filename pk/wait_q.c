...
wait_queue_add(wait_queue_t *queue, wait_t *wait) {
    kassert(list_empty(&(wait->wait_link)) && wait->proc != NULL);
...
wait_queue_del(wait_queue_t *queue, wait_t *wait) {
    kassert(!list_empty(&(wait->wait_link)) && wait->wait_queue == queue);
...
void
wakeup_first(wait_queue_t *queue, uint64_t wakeup_flags, bool del) {
    wait_t *wait;
...
void
wakeup_queue(wait_queue_t *queue, uint64_t wakeup_flags, bool del) {
    wait_t *wait;
    if ((wait = wait_queue_first(queue)) != NULL) {
        if (del) {
            do {
