...
size_t phdr_size = eh.e_phnum * sizeof(Elf_Phdr);
  ...
  int flags = MAP_FIXED | MAP_PRIVATE;
   ...
   return;
fail:
  panic("proc pid fall  open ELF program: %s!", fn);
   
}
void load_elf(const char* fn, elf_info* info)
{

  printk("proc load elf %16lx\n",(uintptr_t)root_page_table);
  file_t* file = file_open(fn, O_RDONLY, 0);
  if (IS_ERR_VALUE(file))
    goto fail;
...
//  printk("proc  load_elf_user %16lx\n",(uintptr_t)pagetable);
  file_t* file = file_open(fn, O_RDONLY, 0);
  if (IS_ERR_VALUE(file))
    goto fail;
  ...
  for (int i = eh.e_phnum - 1; i >= 0; i--) {
    if(ph[i].p_type == PT_LOAD && ph[i].p_memsz) {
      ...
      int flags2 = flags | (prepad ? MAP_POPULATE : 0);
      int prot = get_prot(ph[i].p_flags);
      if (__do_mmap_user(vaddr - prepad, ph[i].p_filesz + prepad, prot | PROT_WRITE, flags2, file, ph[i].p_offset - prepad,pagetable) != vaddr - prepad)
        goto fail;
     //   memset((void*)vaddr - prepad, 0, prepad);
      // if (!(prot & PROT_WRITE))
      //   if (do_mprotect(vaddr - prepad, ph[i].p_filesz + prepad, prot))
      ...
      size_t mapped = ROUNDUP(ph[i].p_filesz + prepad, RISCV_PGSIZE) - prepad;
      if (ph[i].p_memsz > mapped)
        if (__do_mmap_user(vaddr + mapped, ph[i].p_memsz - mapped, prot, flags|MAP_ANONYMOUS, 0, 0,pagetable) != vaddr + mapped)
          goto fail;
    }
  }
