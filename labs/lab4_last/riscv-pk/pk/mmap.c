...
获得二级页表项
/* __walk_internal - get pte and return the address of this pte for addr
                   - if the PT contians this pte didn't exist, alloc a page for PT
 * @addr: the address need to map
...
static pte_t* __walk_internal(uintptr_t addr, int create)
{
  /* LAB3:EXERCISE2 YOUR CODE
   Some Useful MACROs and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
     *   root_page_table : the address of PDT
     *   pt_idx(addr, lever) : get VPN[lever] of the addr 
     *   PTE_V : means the PTE is Valid
     *   __continue_walk_create(addr, pte_t* pte) : create pte(PT) for the addr
     *   pte_ppn: get the ppn part of a pte 
  */
  pte_t* pte = NULL;
  pte_t* pt = NULL;
  //(1) set pt the address of PDT (root_page_table)

  for(int i =1;i>0;i++){
    //(2) get vpn[i] the addr
    //(3) check if pte is Valid
        // if not, need to create a pte 
    //(4) reset the pt 
  }

  //(5) get final pte from pt
  return pte;
}
